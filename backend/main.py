...
# v2t.set_credentials_from_json('/app/credentials/openai_key.json')
v2t.start_client()

s3_path = '/app/credentials/voice_to_image_accessKeys.csv'
sd_path = '/app/credentials/stability-ai-api-key.json'
...
async def upload_audio(file: UploadFile = None):
    ...
    try:
        ...
        file_path = f"/app/data/output/records/{filename}"
...
async def transcribe_audio():
    ...
    try:
        file_path = f"/app/data/output/records/{last_file}"
...
async def generate_image_endpoint(prompt: Prompt):
    ...
    file_csv = '/app/data/data_all.csv'
